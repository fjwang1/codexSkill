...
def _check_bilibili_upload_metadata(project_dir: Path, title: str) -> list[CheckResult]:
	...
	has_timestamp_chapter = re.search(r"\b\d{1,2}:\d{2}(?::\d{2})?\s*[-—至~]\s*\d{1,2}:\d{2}", description) is not None
	if "先行提要" in description and "章节" not in description and not has_timestamp_chapter:
		results.append(CheckResult("PASS", "B 站简介包含先行提要且不包含章节时间轴"))
	else:
		results.append(CheckResult("FAIL", "B 站简介应包含先行提要，且不得包含章节或时间轴"))
...
def _title_attribution(project_dir: Path, title: str) -> tuple[dict[str, Any], CheckResult | None]:
	...
	if not publication:
		return {"publication": None, "used_in_video_title": False, "evidence": None}, None
	chinese_publication = _chinese_publication_name(publication)
	title_prefix_match = TITLE_PREFIX_RE.match(title)
	title_prefix = title_prefix_match.group(1) if title_prefix_match else ""
	if chinese_publication:
		expected_prefix = f"《{chinese_publication}》："
		used = title.startswith(expected_prefix)
		attribution = {
			"publication": chinese_publication,
			"publication_original": publication if publication != chinese_publication else None,
			"used_in_video_title": used,
			"evidence": "source/source_metadata.json publication",
		}
		if used:
			return attribution, CheckResult("PASS", f"标题声明中文文章来源：{expected_prefix}")
		return attribution, CheckResult("FAIL", f"标题缺少中文来源前缀：应以 {expected_prefix} 开头")
	used = bool(title_prefix and _has_cjk(title_prefix) and not _has_ascii_alpha(title_prefix))
	attribution = {
		"publication": title_prefix or None,
		"publication_original": publication,
		"used_in_video_title": used,
		"evidence": "source/source_metadata.json publication; unmapped English publication requires Chinese title prefix",
	}
	if used:
		return attribution, CheckResult("PASS", f"标题使用中文来源前缀：《{title_prefix}》：；原始来源 {publication} 未在映射表中")
	return attribution, CheckResult("FAIL", f"标题来源前缀必须中文化，不能保留英文 publication：{publication}")
...
def _check_preferred_chinese_names(project_dir: Path, srt: Path | None, ass: Path | None) -> list[CheckResult]:
	...
	if not publication or not raw_terms:
		return results
	chinese_publication = _chinese_publication_name(publication)
	publication_offenders: list[str] = []
	for path in _audience_text_targets(project_dir, srt, ass):
		if not path.exists():
			continue
		raw_term = _contains_raw_publication(_read_text(path), raw_terms)
		if raw_term:
			publication_offenders.append(f"{path.relative_to(project_dir)} contains {raw_term}")
	if publication_offenders:
		expected = chinese_publication or "中文来源名"
		results.append(CheckResult("FAIL", f"观众可见文稿/字幕仍有英文来源名，应改为{expected}：{publication_offenders[:8]}"))
	else:
		results.append(CheckResult("PASS", "来源中文化通过：文稿、时间轴和字幕未保留 raw English publication"))
	return results
...
def main() -> int:
	...
	script = _find_text(project_dir, args.script, ("single_host_script", "podcast_script", "chinese_podcast", "口播", "播客", "文稿"))
	...
	if script is None:
		results.append(CheckResult("FAIL", "缺少 single_host_script.md / podcast_script.md 或可识别的中文口播/播客文稿"))
	else:
	...
	schema_version = "english-article-chinese-single-host-video.qa.v3" if voice_mode == "single_host" else "english-article-chinese-podcast-video.qa.v3"
	manifest = {
