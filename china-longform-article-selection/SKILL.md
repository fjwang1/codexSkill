...
使用本 skill 为中文内容生产做“指定日期英文外媒中国爆款候选 Top 5”筛选。
...
`china_viral` 也可理解为用户口语里的 China Viva。选择中国强相关且最可能做成 B 站爆款视频的文章，核心判断只有两件事：

```text
1. 是否一定和中国有关，且不是背景提及。
2. 是否可能成为爆款视频：是否贴近中国社会热点、社会情绪、重点关注问题、流量词和可争议话题。
```

模式判定：

```text
任何“中国选题”“中国热点”“爆款”“流量”“默认选题”请求 -> china_viral
旧 china_domestic 和 asia_china 已停用，不作为日常模式；不要自动回退。
```
...
requested_count: 默认 5；当前可用正文门禁版必须输出 Top 5，除非用户明确要求其他数量
selection_mode: 固定 china_viral
preview_only: 当前默认 false；本 skill 必须自动给出最高分候选作为本轮默认答案
retrieval_gate: 默认 true；进入 Top 5 前必须通过正文可获取性门禁
```
...
本 skill 必须在输出 Top 5 之前确认候选文章有可阅读材料。不要让“看起来很爆但拿不到正文”的文章进入 Top 5。

执行顺序：

```text
1. 先做候选召回和初筛评分，形成高于 requested_count 的候选池。
2. 对按分数排序后可能进入 Top 5 的候选逐篇执行 retrieval gate。
3. 候选通过 gate 后才可进入 Top 5。
4. 如果候选失败，记录排除原因，然后继续检查下一篇候选，直到得到 requested_count 篇可用候选或候选池耗尽。
```
...
3. 必须搜索合法公开同文转载源。
4. 合法源找不到、不可读、或无法确认同文时，候选标记 retrieval_unavailable 并从 Top 5 淘汰。
```
...
5. 不能确认一致时，按失败处理，不要为了凑 Top 5 放行。
```

## 固定来源

只从以下来源检索，除非用户明确修改白名单：
...
如果文章高度相关但长度无法确认，可以进入 retrieval gate；只有抓取或合法公开源验证后确认为可读材料，才允许进入 Top 5。
...
```text
china_relevance_score < 15：不得进入 Top 5
viral_potential_score < 30：不得进入 Top 5
```
...
2. 如果某候选已经在同一 target_date 被制作过，默认从 Top 5 中降权；如果候选质量显著更高且用户可能仍想看，允许保留但必须标注 already_selected=true。
...
```text
/Volumes/GT34/daily_china_article_video/{YYYY-MM-DD}_china_viral/selection/
```
...
length_basis
date_confidence
possible_issues: ["可能敏感", "中国只是部分主题", "归档来源", "合法转载源", ...]
topic_penalties: [{"type": "religion|national_leader", "points": 0-15, "reason": "..."}]
penalty_points: 0-30
...
`ranked-top5.json` 顶层字段：

```text
target_date, selection_mode, requested_count, returned_count,
...
`top_candidates[]` 必须按 rank 排序，`candidate_id` 固定为 `A1` 到 `A5`，并包含“输出要求”中的同一字段集。
...
目标日期：YYYY-MM-DD
当前状态：AI 已完成 Top 5 排名，已为每个候选确认可读文章材料，并已自动选出最高分候选。
...
最终回复给用户时只输出最高信号内容：已完成的 `target_date`、自动推荐的最佳候选 ID/标题/来源/链接/材料来源/推荐理由/分数、A1-A5 简表、`ranked-top5.json` 路径、`selection-decision.json` 路径和 `selection-result.md` 路径。必须明确“已自动选择最高分候选，不需要人工选择；后续制作应使用该候选的 capture_output_path”。
