...
- On this machine, VPN can make Bilibili video-file upload extremely slow. If all upload fields are already filled and only file transfer is pending, a parent workflow may temporarily disconnect VPN, but it may only hard-pause the 5-minute primary reconnect task: run `/bin/bash /Users/wangfangjia/.openclaw/workspace/scripts/vpn-auto-reconnect.sh pause` and verify `/Users/wangfangjia/.openclaw/workspace/scripts/.vpn-auto-reconnect-paused` exists. Never modify, pause, unload, or rewrite the hourly backup LaunchAgent/script: `/Users/wangfangjia/Library/LaunchAgents/com.user.cisco-vpn-auto-hourly-backup.plist` and `/Users/wangfangjia/.openclaw/workspace/scripts/vpn-auto-reconnect-hourly-backup.sh`. Do not treat `StartInterval=600` plus `RunAtLoad=false` as a pause; that only delays reconnect. Arrange redundant thread wakeups before disconnecting. After upload/submission finishes, restore the primary plist defaults (`StartInterval=300`, `RunAtLoad=true`), reload the primary LaunchAgent if this workflow unloaded it, then run `/bin/bash /Users/wangfangjia/.openclaw/workspace/scripts/vpn-auto-reconnect.sh resume`. If primary pause verification fails, do not disconnect VPN; report the upload as BLOCKED. Do not use this VPN workaround once the page already shows `上传完成`; then proceed with one-time verification and submission instead of polling. The hourly backup may reconnect VPN during very long uploads by design; do not disable it.
