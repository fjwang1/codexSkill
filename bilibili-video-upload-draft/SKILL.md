...
description: "Use when the user wants Codex to prepare a Bilibili Creator Center video submission draft in the user's already-logged-in Google Chrome session: open a fresh upload tab, upload a local video file and cover image from absolute paths, fill title/description/tags/category fields, verify the data stuck, and stop before clicking the final submit/post button."
---

# Bilibili Video Upload Draft

Prepare a Bilibili video upload draft in the user's real logged-in Chrome session. This skill is for data entry only. Never click the final `投稿`, `立即投稿`, `提交`, `发布`, or equivalent final submission button; leave the tab open for the user to review and submit manually.

When this skill is called from a video production pipeline such as `english-article-chinese-podcast-video`, `english-article-chinese-single-host-video`, or `worldview-china-podcast-agent`, it is the final publication-preparation phase of that pipeline. It must still stop before final submission, but it must create machine-readable handoff/blocker artifacts in the video project so the parent skill can gate completion.

## Invocation Boundary

This skill is the only allowed implementation boundary for Bilibili upload draft work in the video production pipelines. Upstream skills and workers may prepare `bilibili_upload_metadata.json`, but they must not hand-roll Bilibili upload automation, manually create `bilibili_upload_draft_report.json`, or replace this skill with ad hoc Chrome/Playwright snippets.
...
When a project directory can be inferred from the metadata path or explicit video path, write reports to:
...
These report files are required even when the attempt is blocked before upload. If no project directory can be inferred, write the same report fields to the final response and prefer an explicit output path supplied by the caller.
...
- Use a fresh Chrome tab for every draft. Do not reuse a tab that has a failed or partially submitted state unless the retry is for the same field inside the same draft.
...
- Cover crop confirmation buttons such as `完成` are safe because they only save the cover. The final video submission buttons `投稿`, `立即投稿`, `提交`, or `发布` are never safe for this skill.
- If a modal, toast, or validation error appears, snapshot the visible text, retry only the failed phase, and keep the draft tab open. After the same field fails three times with the same error, stop and report the blocker with current accepted values.
...
- Preserve both the requested and effective wall-clock times in the handoff report, for example `scheduled_publish_at_requested="2026-06-20 17:00 Asia/Shanghai"` and `scheduled_publish_at_effective="2026-06-21 17:00 Asia/Shanghai"`.
...
Do not click final `投稿` / `立即投稿` / `发布` after setting a schedule. The schedule is only part of the draft data entry; the user still performs final confirmation.
...
Before handing the tab to the user, verify:
...
- The final submit/post button has not been clicked.
- The handoff report records `final_submit_clicked=false`.

If any field does not stick, retry that field once using the visible control path, then re-check. If it still does not stick, leave the tab open and report the exact field that needs manual correction.

## Handoff Artifacts

Always write a JSON report before returning to the parent workflow.

Successful handoff shape:
...
```json
{
  ...
  "cover_path": "...",
  "chrome_surface": "chrome:control-chrome",
  "blocked_phase": "open_upload_page | video_upload | metadata_fill | cover_upload | scheduled_publish | verification",
  "retry_count": 0,
  "visible_url": "...",
  "visible_title": "...",
...
Also write a concise Markdown report with the same status, paths, accepted fields, blocker, and next step. The Markdown report is for human review; the JSON report is the parent skill gate input.

## Handoff

Finalize Chrome with the upload tab kept as `handoff` so the user can review and click submit manually:
...
Report the live tab state and the paths used. Never close a tab containing an in-progress Bilibili draft unless the user explicitly asks.
