...
- On this machine, VPN can make Bilibili video-file upload extremely slow. If all upload fields are already filled and only file transfer is pending, a parent workflow may temporarily slow the VPN auto-reconnect interval and disconnect VPN, but it must do so safely: set `/Users/wangfangjia/Library/LaunchAgents/com.user.cisco-vpn-auto.plist` to `StartInterval=600` and `RunAtLoad=false` before reloading it, otherwise the agent can reconnect immediately; arrange redundant thread wakeups before disconnecting; after upload/submission finishes, restore `StartInterval=300` and `RunAtLoad=true`. Do not use this VPN workaround once the page already shows `上传完成`; then proceed with one-time verification and submission instead of polling.
