...
Exception for multi-episode pipelines: when the parent workflow explicitly passes or states `submission_mode=upload_ready_only`, do not click the final submit/publish control. Instead stop after all upload/field/schedule verification gates pass, write `status="READY_TO_SUBMIT"`, keep the Chrome tab open as handoff, and let the parent workflow later call the same skill or browser handoff for `submission_mode=final_submit_existing_ready_tab` / unified final submission.
...
```json
{
  ...
  "submission_mode": "submit_now | upload_ready_only | final_submit_existing_ready_tab"
}
```

`scheduled_publish_at` is optional. This upload skill does not decide which topic belongs to the 11:00 or 17:00 daily slot; that policy belongs to the upstream orchestration skill. It does normalize the provided time at upload time: keep a future exact time unchanged, but if a daily slot time has already passed, roll it forward to the next occurrence of the same wall-clock slot. If no scheduled time is provided, leave Bilibili's timing controls unchanged and report that no schedule was requested. Scheduled submissions still proceed through the same final submission gate after the schedule is verified, unless `submission_mode=upload_ready_only` was explicitly requested.

`submission_mode` defaults to `submit_now`. Use `upload_ready_only` only when the parent series workflow needs to overlap long Bilibili upload/processing time with later episode production and explicitly plans a unified final submit after all episodes are uploaded. Use `final_submit_existing_ready_tab` only to resume a tab that this skill previously left in `READY_TO_SUBMIT`; before clicking submit, re-run the Verification Gate against the current page.
...
Exception for interrupted or resumed series workflows: before starting a new file upload for an episode that may already have been uploaded, inspect all existing Bilibili upload tabs and any explicitly resumed draft page. If a page title field matches the target episode and the upload area shows `上传完成` / `final_video 上传完成`, reuse that page. Only update stale metadata fields such as description, cover, tags, creation declaration, category, and scheduled publish time, then continue to the Verification Gate and Final Submission Gate. Do not click `再投一个` or choose the local MP4 again until you have ruled out an existing uploaded draft for that episode.
...
- In resumed multi-episode work, do reuse an already-uploaded episode draft when the title and upload-complete evidence match the intended episode. Re-uploading a large MP4 wastes time and can create duplicate drafts; field repair should happen in-place on the existing uploaded draft.
...
- On this machine, VPN can make Bilibili video-file upload extremely slow. If all upload fields are already filled and only file transfer is pending, a parent workflow may temporarily disconnect VPN, but it may only use a bounded pause for the 5-minute primary reconnect task: run `/bin/bash /Users/wangfangjia/.openclaw/workspace/scripts/vpn-auto-reconnect.sh pause-for 600` and verify `/Users/wangfangjia/.openclaw/workspace/scripts/.vpn-auto-reconnect-paused` exists and contains `until_epoch=...`. Then disconnect Cisco VPN and arrange redundant thread wakeups at about 10/11/12 minutes to reclaim the upload tab and poll for `上传完成`. The expectation is that after the 10-minute TTL expires, the next 5-minute LaunchAgent tick runs the normal script path, automatically removes the expired pause marker, and reconnects VPN if still disconnected. Do not use bare `pause` for this upload workaround unless `pause-for` is unavailable; bare `pause` is an indefinite manual hold and must be followed by a guaranteed `resume` in the same turn. Never modify, pause, unload, or rewrite the 40-minute backup LaunchAgent/script, whose legacy filenames remain `/Users/wangfangjia/Library/LaunchAgents/com.user.cisco-vpn-auto-hourly-backup.plist` and `/Users/wangfangjia/.openclaw/workspace/scripts/vpn-auto-reconnect-hourly-backup.sh`. Do not treat `StartInterval=600` plus `RunAtLoad=false` as a pause; that only delays reconnect. After upload/submission finishes, verify the primary plist defaults (`StartInterval=300`, `RunAtLoad=true`) and run `/bin/bash /Users/wangfangjia/.openclaw/workspace/scripts/vpn-auto-reconnect.sh resume` only if a pause marker still exists. If bounded primary pause verification fails, do not disconnect VPN; report the upload as BLOCKED. Do not use this VPN workaround once the page already shows `上传完成`; then proceed with one-time verification and submission instead of polling. The 40-minute backup may reconnect VPN during very long uploads by design; do not disable it.
...
- If `metadata.workflow=="worldview-china-podcast-agent"` and `schedule_source=="series_daily_11_17_balanced_ordered_slots"`, do not independently roll over a single episode. This schedule is owned by the parent series workflow because multiple episodes may share `11:00` or `17:00`; rolling only one episode can invert the series order. If the requested time is not in the future, stop before changing timing controls and report `blocker.code="SERIES_SCHEDULE_ROLLOVER_REQUIRED"`, with the next step: parent must rewrite every episode's schedule to the same next valid series publish date and then retry upload.
...
After the Verification Gate passes, branch by `submission_mode`.

For `submission_mode=upload_ready_only`:

1. Confirm a single visible enabled final submission control whose text is one of `立即投稿`, `投稿`, `发布`, or `提交` is present, but do not click it.
2. Record `status="READY_TO_SUBMIT"`, `submission_status="ready_to_submit"`, `final_submit_clicked=false`, and `ready_to_submit=true`.
3. Record enough handoff information to find the tab later: `bilibili_upload_url`, `visible_url`, `visible_title`, `browser_tab_handoff` if available from the Chrome tool, and visible button text.
4. Keep the tab open. Do not close or navigate away from it.
5. Return to the parent workflow so another episode can continue production while this ready tab is preserved.

`READY_TO_SUBMIT` is an intermediate series state only. It is not a formal publication success and must not be accepted by final series QA as complete.

## Final Submission Gate

For `submission_mode=submit_now` or `submission_mode=final_submit_existing_ready_tab`, after the Verification Gate passes, submit automatically. This is the only phase where final publish controls may be clicked.
...
Ready-to-submit intermediate shape:
...
```json
{
  ...
  "created_at": "ISO-8601",
  "project_dir": "/Volumes/GT34/world_and_china_podcast/<project>",
  "metadata_path": "/Volumes/GT34/world_and_china_podcast/<project>/bilibili_upload_metadata.json",
  "video_path": "/Volumes/GT34/world_and_china_podcast/<project>/video/final_video.mp4",
  "cover_path": "/Volumes/GT34/world_and_china_podcast/<project>/cover/cover_4k.png",
  "bilibili_upload_url": "https://member.bilibili.com/platform/upload/video/frame?spm_id_from=333.33.top_bar.upload",
  "chrome_surface": "chrome:control-chrome",
  "tab_status": "ready_to_submit",
  "video_upload_status": "accepted",
  "cover_upload_status": "accepted",
  ...
  "final_submit_button_text": "投稿",
  "final_submit_clicked": false,
  "browser_tab_handoff": {
    "status": "open",
    "note": "Keep this upload tab open for unified final submission."
  },
  "blocker": null
}
```
...
```json
{
  ...
  "blocker": {
    "code": "CHROME_CONTROL_TOOL_UNAVAILABLE | CHROME_EXTENSION_UNAVAILABLE | FILE_URL_ACCESS_NOT_ALLOWED | BILIBILI_LOGIN_REQUIRED | SCHEDULE_IN_PAST | SERIES_SCHEDULE_ROLLOVER_REQUIRED | FIELD_DID_NOT_STICK | PAGE_CHANGED | UNKNOWN",
