...
description: "Check Bilibili post-submission audit status for Worldview China podcast/video runs, including the 20-minute retry loop after SUBMITTED reports, extraction of returned/rejected issue details from Bilibili Creator Center, and writing audit monitor reports/rejection lessons that drive repair and resubmission."
---
...
1. Confirm the upload report has `status="SUBMITTED"` and final submit evidence. If not, write `status="BLOCKED"` with `blocked_phase="precondition"` and stop.
2. Wait 20 minutes after the recorded submission time or, if the submission time is already more than 20 minutes ago, check immediately.
...
6. If `REVIEWING` on the first check, wait another 20 minutes and check once more.
...
- `BLOCKED` or `UNKNOWN`: parent must report exact blocker and keep the progress tab open.
