...
若用户明确要求优化中文音频和源视频画面节奏，可启用 `visual_sync=turn_retimed_basic_v1`：音频仍按 VibeVoice `locked_multi_speaker_roster` 生成，不拆成逐人 single TTS；视频合成阶段根据源视频 speaker turn map 和最终中文音频 turn 时间轴，把每个源 turn 的画面裁到对应中文 turn 时长。该模式只做基础版画面检测：低分辨率帧差 motion score、FFmpeg scene cut 保护、可选原音频静音/口癖候选；不做人脸识别、嘴型同步、手势理解或图表语义理解。目标是“谁在说话和节奏大致对齐”，不是 lip-sync。
...
- 正式完整跑前，建议先用 1-2 个 VibeVoice chunk 做试听验收；若中文/英文混杂、口音异常或任意 speaker 音色混淆，先修 02b clip 或 02c target prompt，不要直接跑全片。
...
如果 03c FAIL，不得进入 03d/03b/04/04b/05。必须回到 03 修源字幕清洗、ASR 术语修正或中文翻译表达，然后重新运行 03c。
...
- 声学一致性门禁：用 02c 中文 voice prompt wav 作为 reference，从最终 `audio/final_podcast.wav` 按 `audio/dialogue_timeline.json` 抽样，每个 chunk 首尾、所有 repair chunk 和每个 speaker 随机片段都要覆盖。抽样片段提取轻量声学 embedding，和两个 reference 算相似度；如果某段更像其他 speaker，或持续低置信，FAIL/REVIEW。
...
- 再为每个 turn 生成 retime edit plan：`target_duration = audio_turn.end - audio_turn.start`，若 `source_duration - target_duration <= 0.8s` 则不裁；否则优先裁源 turn 内的静音/低能量候选、ASR 口癖/重复候选、低 motion 中段，最后才用普通中段 fallback。不得只裁 turn 结尾，也不得全片均匀抽短。
- 保护规则：turn 开头和结尾各 `0.8s` 默认不裁；scene cut 前后 `0.5s` 不裁；不得生成小于 `1.2s` 的保留片段；平均 cut density 不应超过 `10 cuts/minute`。
...
if turn_retimed_basic_v1 is enabled, retime_edit_plan.json status == pass, protected_range_violation_count == 0, min_kept_segment_duration_sec >= 1.2, abs(duration_delta_vs_target_sec) <= 0.75, and cuts_per_minute <= 10
...
- B 站若提示 `定时发布时间不可用`，通常是预约时间距离当前不足 5 分钟或已过期。此时先按用户指示改到可用未来时间，并保持系列间隔一致，例如第一集从 `18:00` 改为 `18:30` 后，第二集从 `19:00` 顺延为 `19:30`。
...
- 大文件上传受 VPN 明显影响时，只在“视频文件已选中、标题/简介/标签/封面/定时等字段已填好，只剩等待传输完成”的阶段临时处理 VPN。临时处理顺序：先给当前 Codex 线程设置 10/11/12 分钟左右的唤醒冗余；再把 `/Users/wangfangjia/Library/LaunchAgents/com.user.cisco-vpn-auto.plist` 改为 `StartInterval=600` 且 `RunAtLoad=false`，bootout/unload 后 bootstrap/load；确认没有立即触发 `vpn-auto-reconnect.sh`；再执行 Cisco VPN disconnect 并确认状态为已断开。`RunAtLoad=false` 必须先改，否则 launch agent 会在重新加载时立刻重连。
- 上传完成或全系列提交后，必须把同一 plist 恢复为默认 `StartInterval=300`、`RunAtLoad=true`，重新加载 launch agent，让 Codex 的 VPN 保障回到常态。若页面已经明确显示 `上传完成`，不要再做无意义上传进度轮询，直接做一次字段/封面/定时/提交证据验证。

### 12 Series Final QA

如果本轮启用了 `04b-series-episodes`，所有 episode 完成 09/10/11a，且 11b 统一最终提交完成后，必须在父 run 上运行系列最终门禁：
...
默认正式生产要求每个 episode 的 `bilibili_upload_draft_report.json.status == SUBMITTED`。`READY_TO_SUBMIT` 只允许作为 11a 和跨 episode 重叠生产的中间态，不能让 `12 Series Final QA` 通过。只有调试或用户明确允许“上传阻塞也先验收机械链路”时，才可传 `--allow-blocked-upload`；该模式不得声称正式发布完成。
