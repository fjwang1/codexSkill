...
- 逐集执行完整链路：`02d title/cover -> 05 VibeVoice -> 06 audio alignment -> 07 subtitles -> 08 source video revoice -> 09 final QA -> 10 Bilibili metadata -> 11 Bilibili upload publish`。
...
report.status is SUBMITTED or BLOCKED
if SUBMITTED, report.final_submit_clicked == true
if SUBMITTED, report.video_upload_status == accepted
if SUBMITTED, report.cover_upload_status == accepted
if SUBMITTED, report.submission_status == submitted
if SUBMITTED, report.field_verification.title == matched
if SUBMITTED, accepted_tags matches or reasonably explains Bilibili rejected/capped tags
