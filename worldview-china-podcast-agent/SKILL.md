...
正式生产默认必须做基础版逐回合原画面同步：`visual_sync=turn_retimed_basic_v1`。这不是 lip-sync，也不要求中文关键词逐词卡到原视频口型；但必须根据源视频 dialogue turn map 和最终中文 audio turn timeline，把每个源 turn 的画面裁剪或延长到对应中文 turn 时长。正式 run 不得以 `visual_sync=disabled_v1` 通过 09 Final QA。

`turn_retimed_basic_v1` 的音频仍按 VibeVoice `locked_multi_speaker_roster` 生成，不拆成逐人 single TTS；视频合成阶段只做基础版画面检测：低分辨率帧差 motion score、FFmpeg scene cut 保护、可选原音频静音/口癖候选；不做人脸识别、不做嘴型同步、不理解图表语义。目标是“每个 turn 边界和 speaker switch 不明显错位”。如果 retime plan 无法 PASS，必须回到 source turn map、cold-open 处理、翻译/分集 source_start/source_end、06c 中文 turn timeline 或 08 retime 策略修复，不得自动降级到整段源视频裁剪。

`visual_sync=disabled_v1` 只允许作为显式调试、历史回放或用户明确要求“不需要一一对应/后半部分直接裁掉/裁到中文音频结束”的例外产物；这类产物必须在 manifest 里标记例外，不能写入正式发布完成历史，不能通过正式 09 Final QA。只有用户明确要求“画面一模一样”“不要硬字幕”“只要 sidecar 字幕”时，才不得烧录字幕，只能输出 sidecar SRT/可选软字幕，并在 render manifest 记录该例外。静态封面/章节卡只允许作为临时试听样片或下载失败时的明确降级验证，不得作为正式目标交付。
...
- 自动选题和历史去重。
- 失败时先阻断下游并保留责任节点证据，但不得把 `FAIL` / `BLOCKED` 当成任务终点；必须进入对应返工 playbook，主动定位、修复、重跑失效门禁，不把缺证据产物标为完成。
...
- `worldview-china-podcast-cold-open-detection` 在源视频下载后、翻译和音色抽取前保守识别片头预告/cold open。只有多信号高置信且裁后开头校验通过，才允许把完整预告片段从 active source 中移除；边界不确定时必须保留原片，不得裁半段。
...
## 失败恢复总原则与节点返工 Playbook

门禁的职责是阻断坏产物进入下游，不是让 automation 结束。任何节点 `FAIL`、`BLOCKED`、`UNKNOWN`、`RETURNED_NEEDS_REPAIR`、脚本异常、浏览器异常或外部服务风控后，执行 agent 必须先按本节定位和返工；只有已经尝试完本节点合理修复路径、记录了证据、仍遇到不可绕过的外部条件时，才可以等待用户介入。

通用规则：

1. 下游必须暂停，但父流程不得结束。先写 `logs/progress.md` 和对应节点 report，记录失败状态、命令、输入 hash、关键错误、已尝试方案和下一步。
2. 每次返工都要备份被替换产物到当前节点的 `repair_attempts/attempt_XXX/`，或者在 report 中写入旧 sha256、新 sha256、修复原因和失效门禁。
3. 先定位 owner node，再修 owner node。不要在字幕、metadata 或上传阶段掩盖 03/04 文本问题；不要在视频阶段掩盖 05/06 音频漏读；不要用 09/12 的 summary 替代具体节点修复。
4. 任一上游产物改变后，所有依赖它的 PASS 都失效。常见重跑链：文本改动后重跑 `03c -> 03d -> 04/04c -> 05-09 -> 10/04c -> 11/11c`；音频改动后重跑 `06 -> 06b -> 06c -> 06d -> 07 -> 04c -> 08 -> 09`；字幕/metadata 改动后至少重跑 `04c -> 09 -> 10/11`。
5. “报告 BLOCKED”是最后动作，不是第一动作。只有登录/验证码/权限、缺少用户提供的外部凭据、候选源天然不合格、主要说话人超过 4 位、全量候选都下载失败、或本机缺失且无法临时安装关键工具时，才向用户要介入。

节点失败的首选修复路径：

- `01/02` 选题、字幕、YouTube 下载失败：先试临时依赖、`yt-dlp` 字幕、`--cookies-from-browser chrome`、常见 Chrome profile、授权 cookies/proxy、完整音频 ASR；若当前候选下载/字幕/画面 form 不可救，回到 shortlist 下一个 `accept` / `backup`，并记录 `source_notes.md`。只有候选耗尽才输出 `NO_ACCEPTABLE_VIDEO` / `SOURCE_VIDEO_DOWNLOAD_BLOCKED`。
- `02` 画面 QA 不通过或主要说话人明显超过 4 位：不要把静态封面、slideshow、新闻包或 5+ 人圆桌硬改成播客；直接 abandon 当前候选，回到 shortlist。若只是 `UNCERTAIN`，主 agent 必须看 opening/middle/end 截图后裁决。
- `02a` speaker census 不确定：查看/抽听 `first_6min_review.mp4` 和 `first_6min.wav`，必要时避开片头广告重新取样，再用 `--confirm-speaker-count` 重跑。确认 5+ 主说话人时换候选，不要合并音色。
- `02b` reference 抽取失败：先补 speaker timeline 或手动指定同一 frozen speaker 的干净 clip，排除广告、音乐、多人重叠、rolling caption；每人不足 25 秒时继续向后找片段。只有源视频确实没有可用单人片段时才换候选。
- `02c` / `05-vibevoice-preflight-audition` 失败：优先修 02b clip、02c `reference_text` 和短中文 `target_text`；重跑 02c 并重新注册 voices；仍不清楚时做 2x2 旧/新脚本和旧/新音色交叉诊断，区分文本问题和 voice prompt 问题。
- `03c` 失败：必须进入 `03r Translation Repair Loop`，回源英文逐段修 ASR 错词、内容缺口、机翻、口癖、数字和中文口语表达，再重跑 03c 与完整阅读审核。
- `03d` / `04c` 文本合规失败：按命中类型分流。单点规范词、逗号数字、具体领导人姓名、标题/metadata 制作话术，回对应文本生成节点直接修；连续涉疆/宗教/地缘政治/平台 lesson 风险，回 `03b` 做完整语义链 cut/bridge，不要关键词替换；标题、封面、字幕或 metadata 新增风险，重生对应节点并复跑 04c。
- `04b` 分集失败：低于 30 分钟短尾集优先合并，高于 40 分钟优先清理重复口语或按语义拆分；修自然 episode subtitle 和 schedule seed 后，重跑相关文本门禁、标题封面和 04c。
- `05` VibeVoice 生成失败、低音量、漏读或卡住：先检查 chunk plan 的 `chunk_count`、`max_chunks_per_episode`、`max_turn_char_count` 和长同 speaker turn；如果 chunk 数超过当前单集目标时长对应上限，先回 04/04b 合并或重划 episode；漏读修复再按 `120-180` 字 subchunk 定点处理。保留 locked roster、voice manifest、speaker_names 和 seed 策略；MPS 异常时记录后切 CPU fallback；只重跑受影响 chunk 后也必须重新拼接母带。
- `06` ASR 不可用：先找本机 `mlx_whisper` / Whisper 环境、临时依赖或可用模型缓存；可以为 ASR 目的分段识别后合并时间轴，但最终验收仍以完整母带覆盖为准。没有任何本地 ASR/forced alignment 可用时才 BLOCKED。
- `06b` 完整音频覆盖失败：用 `repair_targets` 定位 chunk/turn，重跑对应 chunk；同 chunk 反复漏读时拆更小 subchunk；然后重新 `05 assembly -> 06 -> 06b -> 06c -> 06d -> 07 -> 08 -> 09`。
- `06c/07` 时间轴或字幕失败：不要按字数猜时间。回到通过 06b 的 ASR，重建 dialogue timeline；修完整句/语义分句、提前量、悬空断句和单行约束；字幕文本变动后复跑 04c。
- `06d` 音色一致性失败：lineage FAIL 先修 manifest/jobs/repair batch 的 locked roster 配置；acoustic FAIL 再定位到 speaker、chunk 和 repair 段，重跑 02c prompt 或相关 chunk。修后跑完整 06d。
- `08` 合成失败：检查 source episode video、音频/字幕时长、subtitle manifest、Pillow overlay、2K 输出和最终截图；retime 模式失败时重建最新 `turn_audio_timeline` 与 `retime_edit_plan`，不要复用旧 repair 目录。
- `09` / `12` QA 失败：把 `failures[]` 当作 work queue，逐项映射回 owner node 修复并重跑失效门禁。不得只重跑 QA 期待 PASS。
- `10` metadata gate 失败：重生 description/tags/title schedule 字段，确保简介是内容摘要而不是制作说明；metadata 改动后复跑 04c，再进入 11。
- `11` B 站上传/提交 BLOCKED：先复用已有上传完成标签页或草稿，检查登录/验证码、标题匹配、上传完成状态、封面/字段、定时发布时间和 VPN 状态；series 定时过期时整体滚动全系列 schedule；只有需要用户手动登录/验证码或平台不可访问时才等待。
- `11c` 审核退回：把平台时间点和原因写入 lessons，回最终视频/字幕/音频定位完整语义链；优先文本级 cut/bridge 并全链路重跑，已成片且范围明确时才用 `media_level_delete_and_timeline_shift`；重新投稿后从新的提交时间重跑 11c。
- `11c` 审核 BLOCKED/UNKNOWN：先刷新/重开进度页、从稿件管理页按标题/BV 号定位、补写 `bvid`、处理登录/网络/Chrome 问题并重试两次检查；仍无法分类时保留标签页和 exact blocker，再等待用户。
...
/Users/wangfangjia/.codex/skills/worldview-china-podcast-cold-open-detection/SKILL.md
...
├── 02-source-cold-open-detection/
│   ├── cold_open_detection_result.json
│   ├── cold_open_detection_report.md
│   └── frames/
├── 02-source-cold-open-cut/                 # 仅 CUT_CONFIRMED 且校验通过时存在
│   ├── source.cleaned.mp4
│   ├── source.cleaned.wav
│   ├── source_transcript.en.cleaned.json
│   ├── cold_open_cut_validation_result.json
│   └── cold_open_cut_validation_report.md
├── 02a-speaker-census/
│   ├── review/
...
│   ├── pre-tts-frozen-contract-result.json
│   └── pre-tts-frozen-contract-report.md
├── 04b-series-episodes/
...
```json
{
  ...
  "visual_sync": "turn_retimed_basic_v1"
}
```
...
如果用户给定 YouTube URL，可以跳过自动选题，但仍需做 podcast form 和 China relevance 检查；不合格时先说明具体不合格证据，并在用户没有明确要求“只用这个 URL”时回到自动选题寻找替代候选。只有用户指定的唯一 URL 天然不合格或用户禁止替换候选时，才停止等待用户。
...
没有字幕、ASR 或历史缓存文本时，不得进入正式翻译；必须先按失败恢复 playbook 尝试字幕依赖、`yt-dlp` 字幕、cookies 授权重试和完整音频 ASR。当前候选仍不可用时，回到 metadata shortlist 尝试下一个候选；候选耗尽后才停在 metadata shortlist 并写明 `TRANSCRIPT_BLOCKED` / `NO_ACCEPTABLE_VIDEO`。
...
- `1440p` 是优先目标，不是遇阻即停的门禁。若 `available_max_height >= 1440` 但 1440p 下载被 bot/auth/网络/格式合并问题阻断，必须记录 retry/blocker 证据和 `downgrade_reason` 后降到 1080p 继续生产；最终 QA 记 warning，父流程继续定位是否可补救。正式生产硬下限是 1080p，720p/360p 不得作为交付源素材。
...
### 02x Cold Open / Teaser Detection

在 02 下载完整源视频、抽取源音频并规范化英文 transcript 后，必须先检查源视频开头是否是片头预告、前情提要、highlights/cold open 或倒计时引导段，再进入 02a、02b、02c 或 03。读取：
...
- transcript/subtitle 是主信号：先用开头字幕判断是否存在预告结构、正式开场 cue、开头片段在后文重复、以及新开头是否像完整句子。抽帧只用于候选边界附近的确认，不用于随机猜剪点。
- 不得只凭 transcript 判断并 promote active source。正式裁剪必须有强视觉证据或直接视觉审核记录，例如画面出现 `Conversation starts in`、`Podcast starts in`、倒计时、`Coming up`、`In this episode`、大标题卡、预告字幕卡等；如果视觉证据缺失，就保留原片。
- 如果检测结果是 `NO_CONFIDENT_CUT`，这是安全通过状态：保留原始 source，不做任何裁剪，继续 02a/03。
- 如果检测结果是 `NEEDS_VISUAL_REVIEW`，不得无人值守裁剪。主 agent 必须查看 `frames/` 中的 opening/candidate-boundary frames，或启动独立视觉审核 agent，只传源视频前几分钟截图和本 skill，不泄露预期答案。审核结论必须写成 `visual_review.json`，再重跑检测。
- 只有 `cold_open_detection_result.json.status == CUT_CONFIRMED` 时，才允许执行裁剪；`CUT_CONFIRMED` 必须同时满足完整开头段、明确结束边界、视觉证据和 transcript 边界一致，且切点不落在 transcript segment 中间。
...
02-source-cold-open-cut/source.cleaned.sample.mp4
02-source-cold-open-cut/source.cleaned.opening.jpg
```

通过条件：

```text
cold_open_detection_result.json status == CUT_CONFIRMED
cold_open_cut_validation_result.json status == PASS
cut.start_sec == 0
cut.end_sec is within the detected opening range
transcript_validation.status == PASS
first_text_validation.status == PASS
cut boundary does not split any transcript segment
new opening frame and first transcript text do not begin mid-sentence or mid-thought
```

正式 production 只有样片校验 PASS 后才允许 promote active source：
...
`--promote-active` 会先把原始 `source.mp4`、`source.wav` 和 `source_transcript.en.*` 归档为 `*.original_with_cold_open.*`，再用 cleaned 版本替换 active source。若 promote 后任何下游节点发现开头错句、断句、speaker census 异常或 reference 污染，必须立即回滚到 original-with-cold-open 并把 02x 记为 `NO_CONFIDENT_CUT`，不得尝试微调半段裁剪。

禁止事项：

- 禁止裁一半预告。无法确定完整结束点时，保留原片。
- 禁止把 `NEEDS_VISUAL_REVIEW` 当作裁剪许可。
- 禁止在 episode 拆分后才做源视频 cold open 裁剪；必须在 02a/03 前处理 active source。
- 禁止删除正片中的自然介绍、寒暄或节目开场，除非它们被 03 文本规则判定为 sponsor/订阅 CTA，并且只在翻译稿中删，不改源视频。
...
第一次不确定时，可以先不传 `--confirm-speaker-count`。脚本会生成前 6 分钟 review 包并以 `status=needs_review` 暂停下游，主 agent 必须直接看/听 `review/first_6min_review.mp4` 或 `review/first_6min.wav`，确认说话人数和音色后再用确认参数重跑；若片头广告干扰，重新选择更干净的 review window 或补充画面/音频证据。旧参数 `--confirm-two-speakers` 只作为 `--confirm-speaker-count 2` 的兼容别名。正式生产不得跳过这个确认。
...
- 如果 02a 缺失、未冻结、人数不在 1-4、speaker id 不连续或 voice_count 不匹配，02b 必须阻断下游并回到 02a 修复，不得回退到固定预设音色或自动按字幕猜测。
...
- 若 `source_voice_prompts=required`，本节点失败不得继续 02c/05；必须先补 timeline、重选同一 frozen speaker 的干净 clip 或更换候选源视频。正式生产不得回退到固定预设音色。
...
02c-qwen-vibevoice-prompts/voice_distinctness_policy_result.json
02c-qwen-vibevoice-prompts/voice_distinctness_policy_report.md
...
- 02c 生成双人中文 prompt 后、05 preflight 前，必须运行 `02c Voice Distinctness Policy`。只有 exactly 2 speakers 生效；三人或四人播客跳过该双人引用一致性/诊断 policy，继续使用现有 cloned roster 和 06d/09 质量门禁。
- 若 `voice_prompt_policy=qwen_chinese_required`，02c 失败不得继续 05；必须修复 02b clip、reference_text、target_text、Qwen 输出或 voice registration 后重跑。英文或其他非中文源视频必须使用该策略。
...
#### 02c Voice Distinctness Policy

本节点只处理 `qwen_chinese_required` 的双人播客 reference 身份一致性和轻量相似度诊断。执行位置固定在 02c 生成并注册中文 voice prompts 之后、任何 `05-vibevoice-preflight-audition` 或正式 05 VibeVoice 之前。
...
Series episode 模式下，若 episode 通过软链复用父 run 的 `02c-qwen-vibevoice-prompts/voice_prompt_manifest.json`，该 policy 在父 run 跑一次即可；每个 episode 的 preflight/05 会读取同一个已应用 policy 的 manifest。

规则：

- 只对 exactly 2 speakers 生效。`speaker_count == 1` 写 `SKIPPED_SINGLE_SPEAKER_NOT_APPLIED`；`speaker_count == 3` 或 `4` 写 `SKIPPED_MULTI_SPEAKER_NOT_APPLIED`，不得因为某两个 speaker 相似而自动替换多人 roster。
- 阈值仍记录为 `0.90`，但 `spectral_cosine_lightweight_v1` 只作为 broad spectral-envelope 诊断指标，不是校准过的 speaker verification。历史校准显示，已知不同的默认双音色本身也可能得到 `0.99+`，因此相似度分数大于等于 90% 不得单独触发默认音色替换。
- 若双人 prompt 的轻量相似度 `similarity_score >= 0.90`，必须写 `voice_distinctness_policy.status == "WARN_LIGHTWEIGHT_SIMILARITY_HIGH_CLONED_PAIR_KEPT"`、`legacy_threshold_exceeded=true` 和 `fallback_action=none_cloned_pair_kept`，并保留原 cloned pair 继续进入 05 preflight、06d 和 09a/09 质量门禁。
- 只有引用身份层面证明两人实际指向同一份 reference，才硬失败：同一个 registered wav 路径、registered wav sha256 相同，或 02b `source_reference_audio` 路径相同，分别写 `FAIL_IDENTICAL_VOICE_REFERENCE_REQUIRES_REPAIR` 或 `FAIL_IDENTICAL_SOURCE_REFERENCE_REQUIRES_REPAIR`。这类失败必须回到 02b/02c 修 speaker timeline、reference clip 或 Qwen prompt；不得用默认音色掩盖 speaker id 错误。
- 未触发 warning 且引用身份不同，必须写 `voice_distinctness_policy.status == "PASS_ORIGINAL_CLONED_PAIR"`，并保留原 cloned pair。
- `DEFAULT_FALLBACK_APPLIED` 只作为旧 run 兼容或未来显式人工授权 fallback 状态保留；当前无人值守正式生产不得因 `spectral_cosine_lightweight_v1` 高分自动生成该状态。若确实出现该 legacy 状态，manifest 必须保留 `original_cloned_speaker_voices`、`effective_speaker_voices_source == "default_pair_20260618_3"`，并让每个 fallback voice entry 记录 `voice_distinctness_fallback == true`。
- `run_vibevoice_preflight_audition.py` 和 `run_vibevoice_chunks.py` 会在读取双人 02c manifest 时强制检查本 policy；缺失、阈值不是 `0.90`、状态不是 `PASS_ORIGINAL_CLONED_PAIR` / `WARN_LIGHTWEIGHT_SIMILARITY_HIGH_CLONED_PAIR_KEPT` / legacy `DEFAULT_FALLBACK_APPLIED`，或出现 identical-reference FAIL，均不得进入 preflight 或正式 05。
...
正式 episode 已完成 03/04/04c 后，运行本 preflight 前必须先通过 `04d-pre-tts-frozen-contract`。preflight 只验证音色和少量 chunk 的 raw 电平；它不能替代 04d 的文本、speaker roster 和 reviewed hash 冻结。若 04d 缺失、FAIL 或 stale，不得启动 preflight。
...
status == PASS for unattended full 05 generation
...
如果 preflight `status=PASS`，可以进入正式 05 全量 generation。若 `status=YELLOW`，不得无人值守继续；必须先检查 mean_volume、LUFS、silence、ASR 覆盖和抽听 opening/speaker switch，确认没有发虚、闷、噪声、chunk 间响度跳变或音色不稳，并把二级 QA 结论写入本节点报告或日志后才可继续。若 `status=FAIL`，不得启动正式 05 全量 generation。优先按顺序修复：
...
`run_vibevoice_chunks.py` 会在正式非 `--dry-run` 05 入口强制读取上述 preflight 结果；缺失、`FAIL`、`YELLOW` 且无显式二级 QA PASS、`podcast_script.md` hash stale、`voice_prompt_manifest.json` hash stale、voice policy/context policy 不一致、或 preflight chunking policy 与本次正式 05 不一致，都会直接阻断全量 VibeVoice，防止整集生成后才发现 voice prompt 或 chunk 策略问题。

2026-06-26 经验修正：`20260626_1` 的源视频音频和 02c prompt 自身电平都正常，但 VibeVoice raw chunks 中 29/32 个低于当时旧门槛。2x2 交叉诊断显示：昨天稿子×昨天音色 `max=-5.8 dBFS`；昨天稿子×今天音色 `max=-9.6 dBFS`；今天稿子×昨天音色 `max=-5.2 dBFS`；今天稿子×今天音色 `max=-9.5 dBFS`。结论是 voice prompt 能导致 run-wide raw peak 偏低；preflight 必须前置。当前门禁采用三档：`max >= -12 dBFS` 自动通过，`-15 <= max < -12 dBFS` 为 YELLOW 需二级 QA，`max < -15 dBFS` 或 `mean < -30 dBFS` 为 FAIL。
...
- 如果 02x 已把片头预告/cold open 从 active source 移除，03 不得再次静默跳过 active source 的自然开头。特别是 active source 首段同时包含“主持人开场 CTA + 正式问题 + 嘉宾回答”时，必须保留一个短的主持人开场 bridge，删除 CTA 制作话术，并按 frozen roster 拆成 `host opening bridge -> host question -> guest answer`。不得直接把第一段 source_start 改到正题问题处，例如 18 秒，让成片以半截问题开场。若无法确定问题/回答边界，宁可保留自然开头并标记人工复核，不得删半段。
...
- 政治访谈里的 `state` 必须按上下文翻译，不得机械写成 `国家机器`。当原文把 `the state` 与 `White House`、总统、民众、国会或具体部门对比时，通常指美国的政府体系、建制系统、常设官僚体系，或外交/国防/经贸等机构系统；可译为 `美国政府体系`、`美国建制系统`、`美国外交和国防系统`、`美国官方体系`。只有原文明说 `state apparatus`、`state machinery`、`repressive/coercive state apparatus`，且语境确实是暴力/统治机器时，才可译为 `国家机器`。若原文是美国的 `states`，必须译为 `各州/州政府`，不要和 `the state` 混淆。
...
- 对 exactly 2 speakers 且 02a roster 已明确 `host/interviewer` 与 `guest/expert` 的访谈，03 可以修正明显的 source diarization 翻转：短主持人问题归 host，长回答或 answer-like turn 归 guest，并在 `pre_translation_cleanup.speaker_repair` 记录原因。该自动修正只适用于双人访谈；三人/四人 panel 不适用，避免把多个嘉宾压成一个 voice slot。
- 章节切分只服务于后续 TTS 分块：按原视频话题转折、自然段落和时长切分，不重排内容。

#### 03e Speaker-Turn Roster Consistency Gate

03/04 产物进入任何 VibeVoice 前，必须检查 frozen roster、源转写 speaker id、中文稿 turn speaker 和源视频画面是否一致。这个 gate 防的是同一个可见人物或同一段连续 source answer 被分配给两个 voice slots，或主持人问题被嘉宾 voice slot 朗读。

运行：

```bash
python3 /Users/wangfangjia/.codex/skills/worldview-china-podcast-agent/scripts/run_speaker_turn_roster_consistency_gate.py \
  --run-dir <run_dir>
```
...
```text
03e-speaker-turn-roster-consistency/speaker-turn-roster-consistency-result.json
03e-speaker-turn-roster-consistency/speaker-turn-roster-consistency-report.md
```

通过条件：

```text
speaker-turn-roster-consistency-result.json exists
status == PASS
input_file_hashes covers current 02a-speaker-census/speaker_roster.json
input_file_hashes covers current active 03b/03 source_transcript.zh*.json
input_file_hashes covers current 04-podcast-script/script_turns.json and podcast_script.md
```

`run_vibevoice_chunks.py` 会在进入正式 05 前强制检查这个 PASS 和输入 hash；缺失、FAIL 或 stale 都必须先回 03/03b/04 修复并重跑本 gate，不得靠 06d 或 09a 兜底。

最低要求：

- 抽查开头、前 10 分钟内至少 8 个 speaker switch、每个 speaker 至少 3 个 turn，以及任何 source segment 重叠/空文本/长 gap 附近的 turn。
- 对 exactly 2 speakers，必须能明确维持 `Speaker 0` 和 `Speaker 1` 的角色：例如 `Speaker 0=Dr. Samir Saran/guest`、`Speaker 1=Raj Shamani/host`。不得让同一个人一会儿是 `Speaker 0`，一会儿是 `Speaker 1`。
- 如果一个源字幕 segment 混合了“主持人问题 + 嘉宾回答”，必须在 03 或 04 拆成两个 speaker turns；不得为了保留原字幕 segment 而让一个 voice slot 读两个人。
- 如果源 ASR/diarization 把同一长回答交替标成两个 speaker，必须回 03 修正 speaker attribution，并重新生成 `source_transcript.zh.json`、`podcast_script.md`、`script_turns.json` 和后续 chunk plan；不得靠 06d lineage 或 09a 抽查兜底。
- 03e 的主持人问题判定必须保守：英文 `what/why/how/which` 和中文 `为什么/哪些/是不是` 只有在句首构成提问 prompt，或文本里有明确问号时，才作为 host-question 证据。嘉宾回答里的从句，例如 `why I'm saying this` / `我为什么这么说`，不得误判为主持人问题。
- 若无法从字幕和画面稳定判断主持/嘉宾边界，当前候选应标记 `speaker_turn_attribution_uncertain` 并暂停正式 TTS；不要用默认音色或相似度 fallback 掩盖 speaker id 错误。

2026-06-27 经验修正：`20260626_1/episode_002` 的 09a 抽查发现 source diarization 把同一个老嘉宾的连续回答交替分给 `Speaker 0` 和 `Speaker 1`，同时部分 host 问句落到 `Speaker 0`。虽然 06b/06d、字幕和 turn-retime 都可 PASS，但成片会表现为同一画面人物用两个中文音色说话。以后这类问题必须在 03e/04 前置阻断，不能等全量 VibeVoice 和 08 成片后返工。

2026-06-27 经验修正：`20260626_1/episode_001` 的 cold-open detection 本身正确切掉了 0-132.64 秒预告，但后续 03 修复层又把 active source 开头 0-18 秒的主持人开场 CTA/过渡静默跳过，导致成片第一句直接是“美国到底怎么了？”，没有主持人自然开场。以后 cold-open 后的开场 CTA 只能在 03 文本层删制作话术并插入短 bridge，不得通过移动 episode/source_start 让成片以半截正题开场。
...
- 将政治术语按语境翻译，例如 `the state` 在 `White House / state / people` 三分法中应译为“美国政府体系/建制系统/官方体系”，不得直接译为“国家机器”；`50 states`、`go to the states` 则译为“美国各州/走进各州”。
...
- 02x cold-open 后的 active source 首段含主持人开场 CTA、正式问题和回答，但翻译稿从几秒后才开始，或直接以裸主题问题开场、缺少主持人开场 bridge。此检查只适用于父 run 或系列第 1 集；系列第 2 集及以后是中段起播，不得套用全片开头 CTA/bridge 规则。
...
03d-risk-compliance-review/independent-review-result.json
```
...
summary.fail_findings == 0
reviewed_files includes 03-source-translation/source_transcript.zh.json
reviewed_file_hashes covers the current faithful and active translation files
...
如果 03d FAIL，不得进入 03b/04/04b/05，但父流程不得结束。必须先读取 `fail_findings`、命中 rule id、上下文和 reviewed file hash，按问题类型返工：规范称谓、逗号数字、具体领导人姓名、制作话术等确定性问题回 03 源头修复；涉疆/民族宗教/地缘政治/平台 lesson 这类“忠实记录但发布不能播”的连续语义链，进入 03b 做完整上下文的最小必要删改、cut 或 bridge。修复后必须复跑 03c 和 03d，让语义表达和风险合规两个早期门禁同时覆盖安全稿；旧 PASS 不得复用。
...
- 必须另外启动一个独立审核子 agent，使用本 child skill 审核 `03-source-translation`、`03b-mainland-publish-safety`、`04-podcast-script`、`podcast_script.md` 以及当时已存在的标题/封面/字幕/metadata。子 agent 不参与正文生成，只给 `PASS` / `FAIL`、命中位置和建议修复方向。如果当前工具面无法启动独立子 agent，本节点不得静默降级为 deterministic-only；应阻塞并等待可用审核方式或用户明确介入。
- 输出固定为：
...
04c-bilibili-text-compliance/independent-review-result.json
```
...
03d/04c 每次 PASS 都必须写 `reviewed_file_hashes`。最终 QA 只认当前文件 sha256 与审核报告一致的 PASS；只要脚本、字幕、标题、封面、metadata 或安全稿在审核后发生修改，旧审核结果一律失效，必须重跑 03d/04c。音频/字幕 manifest 中的可发布文本字段仍会被扫描；但非发布元数据变化不应单独让 04c 失效。
...
independent-review-result.json exists, status == PASS, and reviewed_file_hashes covers the current key publishable text
```

如果 04c FAIL，不得进入 04b/05/06/07/08/10/11，但必须主动返工。先判断 FAIL 来自哪类当前文件：`podcast_script.md` 语义链风险回 03b/04；字幕风险回 07 并同步显示稿；标题/封面风险回 02d；metadata/简介/标签风险回 10；stale hash 只需要重新审核最新文件。修复后重新运行 deterministic check 和独立审核子 agent，并确保 `reviewed_file_hashes` 覆盖最新文本。
...
### 04d Pre-TTS Frozen Contract And Dependency Rebuild Plan

在任何 `05-vibevoice-preflight-audition` 或正式 `05 VibeVoice Chunks` 之前，必须运行一次 Pre-TTS frozen contract。这个节点不替代 03c、03d、03e 或 04c；它把这些门禁当前 PASS 覆盖的 speaker roster、active translation、script、标题/封面和发布文本 hash 汇总成一个冻结契约，防止上游文本或说话人归属改动后继续复用旧 chunk plan、旧 preflight、旧音频、旧字幕或旧视频。

运行：

```bash
python3 /Users/wangfangjia/.codex/skills/worldview-china-podcast-agent/scripts/run_pre_tts_frozen_contract.py \
  --run-dir <run_dir> \
  --force
```
...
```text
04d-pre-tts-frozen-contract/pre-tts-frozen-contract-result.json
04d-pre-tts-frozen-contract/pre-tts-frozen-contract-report.md
```

通过条件：

```text
pre-tts-frozen-contract-result.json exists
status == PASS
input_file_hashes covers current 02a-speaker-census/speaker_roster.json
input_file_hashes covers current 02c-qwen-vibevoice-prompts/voice_prompt_manifest.json when qwen_chinese_required
input_file_hashes covers current active 03/03b translation JSON/MD
input_file_hashes covers current 04-podcast-script/script_turns.json, 04-podcast-script/podcast_script.md and root podcast_script.md
if video_title.txt or cover/cover_title.json exists, input_file_hashes covers them
03c, 03d, 03e and 04c gates are all PASS and their reviewed_file_hashes match the current files
04c independent review PASS covers the current key publishable text
dependency_rebuild_contract contains text_change, audio_change, subtitle_change and metadata_change chains
```

`run_vibevoice_preflight_audition.py` 和 `run_vibevoice_chunks.py` 会在检测到正式 `04c-bilibili-text-compliance` 产物时强制检查本节点；缺失、FAIL 或 input hash stale 时，必须阻断 05。不得为了省时间删除 04c 或用旧 preflight 绕过本节点。

如果 04d 发现 stale 或 FAIL，先用 dependency-aware planner 生成返工计划，不要靠聊天记忆判断：

```bash
python3 /Users/wangfangjia/.codex/skills/worldview-china-podcast-agent/scripts/plan_dependency_rebuild.py \
  --run-dir <run_dir> \
  --changed-scope auto
```
...
```text
00-rebuild-plan/rebuild-plan.json
00-rebuild-plan/rebuild-plan.md
```

返工链路规则：

- `text_change`：重跑 `03c -> 03d -> 03e -> 04c -> 04d -> 05 preflight -> 05 -> 06 -> 06b -> 06c -> 06d -> 07 -> 04c -> 08 -> 09a -> 09 -> 10 -> 04c -> 11 -> 11c -> 11d`。
- `audio_change`：重跑 `06 -> 06b -> 06c -> 06d -> 07 -> 04c -> 08 -> 09a -> 09 -> 10 -> 04c -> 11 -> 11c -> 11d`。
- `subtitle_change`：重跑 `04c -> 08 -> 09a -> 09 -> 10 -> 04c -> 11 -> 11c -> 11d`。
- `metadata_change`：重跑 `04c -> 11 -> 11c -> 11d`。

本节点的优化目标是减少“发现太晚导致整链路无计划返工”：错误必须前移到 TTS 前；一旦上游文件变化，必须由 rebuild plan 明确失效范围。它不能降低任何质量门禁，不能跳过 03c/03d/03e/04c/06b/06d/09a/09，也不能把 FAIL 当成通过。
...
- 逐集生产链路：`03c translation semantic QA refresh -> 03d early risk compliance refresh -> 03e speaker-turn roster consistency gate -> 02d title/cover -> 04c text compliance refresh -> 04d pre-TTS frozen contract -> 05 preflight audition -> 05 VibeVoice -> 06 final audio ASR -> 06b audio transcript integrity QA -> 06c audio timeline alignment -> 06d voice consistency QA -> 07 subtitles -> 04c text compliance refresh -> 08 source video revoice -> 09 final QA -> 10 Bilibili metadata -> 04c text compliance refresh -> 11a Bilibili upload/ready-to-submit -> 11b final submit when applicable -> 11c Bilibili audit monitor -> 11d post-run process review`。
...
正式 `05-vibevoice-preflight-audition` 和正式 05 前都必须已有当前 `04d-pre-tts-frozen-contract/pre-tts-frozen-contract-result.json` 且 `status == PASS`。`run_vibevoice_preflight_audition.py` 与 `run_vibevoice_chunks.py` 在正式路径上会无条件检查 04d；如果 03/04/标题/封面/04c 在 04d 后有任何改动，必须先用 `plan_dependency_rebuild.py` 生成返工计划并重跑失效门禁，不得直接复用旧 preflight、旧 chunk plan 或旧 05 输出。
...
- chunk 数必须和当前单集目标时长绑定，避免过度碎片化导致情绪和语气连续性丢失。默认比例是 `1 chunk / target minute`；当前 04b 默认单集 `target_minutes_max=40`，所以每个 episode 最多 `40` 个 chunk。若未来把单集目标改成 `20`、`30`、`60` 分钟，`max_chunks_per_episode` 应分别随之变为 `20`、`30`、`60`，不得写死为 40。
- `run_vibevoice_chunks.py` 必须在 `chunk_plan.json` 写入 `max_chunks_per_episode`、`chunk_limit_target_minutes`、`chunk_limit_source` 和 `chunks_per_target_minute`。如果 `chunk_count > max_chunks_per_episode`，不得启动正式 VibeVoice；优先回 04/04b 合并过碎 turn、恢复更完整的语义承接或重新拆 episode。只有 06b 定位到局部漏读时，才允许把少数问题 chunk 拆成更小 subchunk，但修复后总 chunk 数仍必须符合当前目标时长上限。
...
resident batch 报告不得被覆盖成只剩最后一次修复。脚本每次写 `resident_batch_report.json` 前会把旧报告归档到 `05-vibevoice-chunks/resident_batch_runs/`，并维护 `resident_batch_runs/index.json`；完成后会生成 `resident_batch_report.effective_final.json`，按最终 `audio_manifest.chunks` 为每个 chunk 选择最新有效 job 证据。正式 `audio_manifest.resident_batch_report` 必须指向这个 effective final report。若 effective report 不能覆盖所有最终 chunk，05 失败，不得进入 06。

resident batch 结束后必须立刻检查每个本次 job 的 raw wav 是否存在且非空，输出 `resident_batch_output_coverage.json`。若出现 `chunk_016` 这类缺失 raw wav，脚本必须立即 FAIL，并提示只用 `--force-chunk-id <missing_chunk>` 重跑缺口；不得继续长时间等待、不得重跑全量 batch，也不得让后续 postprocess/ASR 才发现缺块。
...
如果 `02c-qwen-vibevoice-prompts/voice_prompt_manifest.json.voice_distinctness_policy.status == "WARN_LIGHTWEIGHT_SIMILARITY_HIGH_CLONED_PAIR_KEPT"`，05 必须继续使用当前 `speaker_voices` 中的 cloned roster；该 warning 只说明轻量 spectral 指标不可分，不允许改用默认音色。若旧 run 或显式人工授权出现 `DEFAULT_FALLBACK_APPLIED`，这里的 `speaker_voices` 已经是 fallback 后的 effective roster，05 必须使用它；不得再读取 `original_cloned_speaker_voices`，也不得把一个默认音色和一个原 cloned 音色混用。双人 02c manifest 缺少 `voice_distinctness_policy`，或 policy 是 identical-reference FAIL 时，preflight/05 必须阻断并回到 02b/02c policy 节点。
...
正式非 `--dry-run` 调用会再次校验 `05-vibevoice-preflight-audition/preflight_audition_result.json` 和当前输入 hash；不要手动跳过这个节点再直接跑全量 VibeVoice。
...
- 跑 `volumedetect`；raw VibeVoice 当前采用三档门禁：`max_volume >= -12 dBFS` 且 `mean_volume >= -30 dBFS` 为 PASS；`-15 dBFS <= max_volume < -12 dBFS` 且 `mean_volume >= -30 dBFS` 为 YELLOW，必须检查 mean_volume、LUFS、silence、ASR 覆盖和抽听，确认没有发虚、闷、噪声或 chunk 间响度跳变；`max_volume < -15 dBFS` 或 `mean_volume < -30 dBFS` 为 FAIL。FAIL 不要靠 loudnorm 硬救，优先重跑该 chunk 或修复 02b/02c voice prompt。
...
如果本地没有可用 ASR/forced-alignment 工具，不得进入 06b、字幕或视频合成；但必须先尝试恢复本地 ASR：检查 `/Users/wangfangjia/code/bilibili-mcp/.venv-asr/bin/mlx_whisper`、可用 Whisper/MLX 环境、模型缓存和临时依赖；必要时可为了 ASR 把最终母带分段识别后合并为完整时间轴。只有确认本机无法运行任何可接受的 ASR/forced alignment 后，才在本 gate 报告 `BLOCKED`。
...
如果 06b 失败，不得进入 06c 字幕时间轴、07 字幕、08 视频合成、10 B 站 metadata 或 11 投稿；必须把 `repair_targets` 当作修复队列。优先重跑定位到的 VibeVoice chunk；同一 chunk 反复漏读、乱序或压缩时，把对应长 turn/长同 speaker 区间拆成 `120-180` 字左右 subchunk，并严格复用 locked roster、voice prompt manifest、speaker_names 和 seed 策略。修复后重新拼接母带并重跑 `06 -> 06b -> 06c -> 06d -> 07 -> 08 -> 09`。
...
08 默认启用 `visual_sync=turn_retimed_basic_v1`，因此本节点必须把 `dialogue_timeline.json.turns` 复制或规范化为 `06c-audio-timeline-alignment/turn_audio_timeline.json`，每个 turn 至少包含 `turn_id` / `turn_index`、`speaker`、`start_sec`、`end_sec`、`alignment_confidence`。该时间轴只来自完整中文母带 ASR/forced alignment，不得按中文字符数比例推断。
...
- 声学一致性门禁：用 02c 中文 voice prompt wav 作为 reference，从最终 `audio/final_podcast.wav` 按 `audio/dialogue_timeline.json` 抽样，每个 chunk 首尾、所有 repair chunk 和每个 speaker 随机片段都要覆盖。抽样片段提取轻量声学 embedding，和 reference 算相似度。若 02c reference prompts 本身可分离，某段最佳匹配到其他 speaker 必须 FAIL。若 02c reference prompts 在轻量 embedding 下高度相似（默认 cosine >= 0.98），且该段对两个已锁定 speaker 的相似度反转 margin 很小（默认 abs(margin) <= 0.03），这只能证明两个 reference 在该 embedding 下不可分，降级为 REVIEW/warning，不得误报为第三音色；但 lineage 错误、默认音色、未锁 roster、或大 margin 身份错配仍必须 FAIL。
...
- `subtitle_manifest.json` 必须写入 `timing_policy.status == PASS`；`segmentation_policy.status` 应尽量为 `PASS`，但仅作为排版 warning，不得单独阻塞后续节点。
- `timing_policy.late_start_violation_count == 0`，`lead_applied_per_split_cue == true`。
- `segmentation_policy.dangling_fragment_violation_count == 0` 是硬门禁；`hard_width_fallback_count`、`line_violation_count`、`style.max_lines` 和 `fits_single_line=false` 是视觉排版 warning，优先返工修好，但不要因为它们单独终止生产。
...
- 正式生产默认使用 `visual_sync=turn_retimed_basic_v1`：先生成 `visual_activity.json` 和 `retime_edit_plan.json`，再渲染 `work/source_retimed_basic.mp4`，最后静音源对白、混入允许复用的非对白源音频片段、叠加中文主音轨并烧录中文字幕。
- 进入 08 前必须先物化当前 episode 的 source dialogue turn map：运行 `scripts/build_source_dialogue_turn_map.py`，输出 `04-source-dialogue-turn-map/source_dialogue_turn_map.active.json` 和 `source_dialogue_turn_map.report.md`。该 map 必须基于当前 active 03b/03 源段、当前 05 `audio/audio_manifest.json` 中的 `source_turn_index/source_part_index`，以及当前 06c `turn_audio_timeline.json` 的真实音频时间轴生成；不得重新抓字幕、不得重新下载源视频。
...
- 正式 B 站交付默认产出 `source_video_revoice_turn_retimed_basic_burned_subtitles` 或 `source_video_revoice_episode_segment_turn_retimed_basic_burned_subtitles`，并把该版本同步为根级 `video/final_video.mp4`。该版本底层画面来自源视频，但经过 turn-level retime 和字幕 overlay 重编码，不再宣称“画面一模一样”。
...
- 正式生产不得只保留源视频原始时间线再裁到中文音频长度。若 retime 失败，应回到 owner node 修复；只有显式调试或用户明确批准的例外产物才可使用 `visual_sync=disabled_v1`，且不能通过正式 09 Final QA。
- 临时试听样片可以用 `--source-start-sec` 和 `--match-audio-duration` 截取源视频片段展示“原视频画面 + 中文音频”效果，但必须在 manifest 中标记 `review_sample=true`，不得写入正式历史。
- 正式 08 不允许全长探索性/手工测试渲染混在 `08-source-video-revoice/work/` 中，例如 `manual_cfr_test.mp4`、`debug_*_test.mp4`、`probe_*` 这类接近正片时长的文件。短样片必须走 review sample 并标记 `review_sample=true`；正式路径只允许一次 turn-retimed video render、一次 subtitle overlay 中间层和一次 final video render。脚本会扫描并在 `render_manifest.json.formal_full_render_experiment_scan` 记录结果；默认 `formal_full_render_experiments_allowed=false`，发现全长 manual/test/debug render 直接 FAIL。

必选 `turn_retimed_basic_v1`：

- 本模式只发生在音频产出和 06c 时间轴完成之后，不改变 05 VibeVoice 音频策略。05 仍必须使用 `locked_multi_speaker_roster`，不得为了视频对齐改成逐人 single TTS。
- 输入必须包含当前 episode 的源视频 dialogue turn map 和最终中文音频 turn timeline。正式 08 只接受当前 episode 的 `04-source-dialogue-turn-map/source_dialogue_turn_map.active.json`；不得再使用历史 `02-source-turn-map` 节点，也不得把 `03b-mainland-publish-safety/source_transcript.zh.safe.json`、`03-source-translation/source_transcript.zh.json` 或 `02-source-capture/source_transcript.en.json` 当作正式 source turn map fallback。中文 turn timeline 默认读取 `06c-audio-timeline-alignment/turn_audio_timeline.json`，fallback 为 `audio/dialogue_timeline.json`，但必须覆盖当前 `audio/final_podcast.wav`。
- `source_dialogue_turn_map.active.json` 中每个 dialogue turn 必须写入 `audio_turn_id` 或 `audio_turn_ids`，把源画面 turn 显式绑定到当前中文音频 turn；08 plan 必须优先使用显式绑定，只有历史/调试 map 缺少绑定时才允许按 speaker 贪心匹配。若 VibeVoice 或 06c 把一个源段拆成多个中文 audio turn，map 应按当前 `visual_audio_end - audio_start` 把源段分摊为多个绑定 turn；中文音频整体更短时可以丢弃源段尾部冗余画面，中文音频更长时再交由 retime 做 `freeze_tail`。
...
- 输出 `08-source-video-revoice/retime_edit_plan.json`，再一次性渲染 `08-source-video-revoice/work/source_retimed_basic.mp4`。正式渲染不得使用 concat demuxer 反复引用同一个长 H.264 源文件并依赖 `inpoint/outpoint` 裁段；这会在非关键帧和 timestamp gap 下产生重复帧/冻结画面。必须用 FFmpeg filter graph 对每个 `video_range` 显式 `trim=start:end,setpts=PTS-STARTPTS`，对 `freeze_tail` 显式生成冻结段，再用 concat filter 拼接为单一输出。渲染后必须检查 `source_retimed_basic.mp4` 时长与 `retime_edit_plan.summary.target_duration_sec` 的差值不超过 `0.75s`。
- 08 还必须运行自动 motion integrity gate：在若干 `source_mode=video_range` 区间内比较最终/retimed 画面上半屏 5-10 秒帧差与映射源视频同区间帧差。如果最终/retimed 近似同帧而源视频明显有运动，08 必须 FAIL；不得把这类冻结画面留给 09a 人工抽查才发现。
- 本模式只承诺画面节奏和说话人 turn 对齐，不承诺嘴型同步。对播客固定机位、双人 split screen、多人圆桌、Zoom 访谈更适用；如果源视频包含大量图表、屏幕共享、新闻 B-roll 或频繁画面语义变化，也不得自动降级为 `disabled_v1`；必须通过 retime plan 和 09a 多模态抽查判断是否可发布，失败则换候选、修 source turn map 或人工决定是否放弃。

先物化 source dialogue turn map：

```bash
python3 /Users/wangfangjia/.codex/skills/worldview-china-podcast-agent/scripts/build_source_dialogue_turn_map.py \
  --run-dir <run_dir> \
  --force
```
...
```text
04-source-dialogue-turn-map/source_dialogue_turn_map.active.json exists
source_dialogue_turn_map.status == PASS
source_dialogue_turn_map.method == episode_audio_manifest_bound_source_segment_visual_duration_fit
source_dialogue_turn_map.audio_manifest_sha256 matches current audio/audio_manifest.json
source_dialogue_turn_map.turn_audio_timeline_sha256 matches current 06c-audio-timeline-alignment/turn_audio_timeline.json or current audio/dialogue_timeline.json
source_dialogue_turn_map.mapped_turn_count equals count of current audio_manifest.turns with source_turn_index
every dialogue map turn has audio_turn_id or audio_turn_ids
```
...
08 渲染完成后、09 Final QA 前，必须运行统一多模态抽查门禁。它不是全量逐帧检查，而是把所有适合抽查的问题集中一次解决，避免音色、字幕、画面语义和 speaker switch 各跑一套昂贵人工检查。

运行：

```bash
python3 /Users/wangfangjia/.codex/skills/worldview-china-podcast-agent/scripts/run_multimodal_spot_qa.py \
  --run-dir <run_dir> \
  --force
```

首次运行会生成抽查包并停在 `NEEDS_REVIEW`：

```text
09a-multimodal-spot-qa/multimodal-spot-qa-package.json
09a-multimodal-spot-qa/multimodal-spot-qa-review-package.md
09a-multimodal-spot-qa/samples/sample_XXX/frame.jpg
09a-multimodal-spot-qa/samples/sample_XXX/context_audio.wav
09a-multimodal-spot-qa/multimodal-spot-qa-result.json
09a-multimodal-spot-qa/multimodal-spot-qa-report.md
```

抽样策略：

- 每集默认抽 `20-24` 个点。
- 必抽 opening、ending、正式开场附近、speaker switch、包含人名/数字/机构/地名/标题卡语义的 anchor cue。
- 其余样本按全片均匀覆盖补齐。采样实现必须强制覆盖全片分位点，尤其是 middle 和 ending，不得因为前半段 speaker switch 或 semantic anchor 候选过多而只保留最早的 20-24 个样本。
- 每个样本包含当前帧、前后约 10 秒音频上下文、当前字幕 cue、前后字幕 cue、当前中文 turn 文本、speaker 和采样原因。
- 样本 package 的 `turn_index`、`speaker` 和 `turn_text` 必须优先来自字幕 cue 的 `source_turn_id` / `source_turn_index` 绑定，再回退到时间点匹配。不得只按 sample time 找第一个覆盖该时间点的 turn；在 turn 边界轻微重叠时，这会把已切到下一说话人的字幕误标为上一 turn，导致 09a 自己产生假失败。
- 09a 必须记录自动 `video_motion_integrity` 检查结果。该检查不是人工审阅替代品，而是冻结画面/重复帧的硬门禁。
...
审核人或独立审核 agent 必须逐样本填写 JSON，每个 criterion 只能是 `PASS`、`WARN`、`FAIL` 或 `NA`；任意 `FAIL`、缺失样本、缺失 criterion 或未确认 `read_entire_package=true` 都阻断 09。审核完成后运行：
...
```text
09a-multimodal-spot-qa/multimodal-spot-qa-result.json exists
status == PASS
summary.sample_count >= 20
summary.reviewed_sample_count == summary.sample_count
summary.fail_count == 0
required_criteria covers visual_audio_semantic_alignment, subtitle_audio_timing, subtitle_text_segmentation, voice_identity_consistency, speaker_switch_coherence
input_file_hashes match current final_video.mp4, final_podcast.wav, render_manifest.json, subtitle_manifest.json, dialogue_timeline.json and 06d voice consistency result
```
...
if voice_prompt_policy=qwen_chinese_required and 02c speaker_count == 2, voice_prompt_manifest.voice_distinctness_policy exists, scope == exactly_two_speakers_only, threshold == 0.90, and status is PASS_ORIGINAL_CLONED_PAIR, WARN_LIGHTWEIGHT_SIMILARITY_HIGH_CLONED_PAIR_KEPT, or legacy DEFAULT_FALLBACK_APPLIED
if voice_prompt_manifest.voice_distinctness_policy.status == WARN_LIGHTWEIGHT_SIMILARITY_HIGH_CLONED_PAIR_KEPT, fallback_action == none_cloned_pair_kept, effective_speaker_voices_source is absent, speaker_voices keeps the cloned roster, and downstream 05/06d/09a must validate the actual voices
if voice_prompt_manifest.voice_distinctness_policy.status is FAIL_IDENTICAL_VOICE_REFERENCE_REQUIRES_REPAIR or FAIL_IDENTICAL_SOURCE_REFERENCE_REQUIRES_REPAIR, final QA must FAIL and the parent workflow must rerun 02b/02c instead of using default voices
if voice_prompt_manifest.voice_distinctness_policy.status == DEFAULT_FALLBACK_APPLIED, effective_speaker_voices_source == default_pair_20260618_3, original_cloned_speaker_voices exists, and every effective speaker_voices entry records voice_distinctness_fallback == true
if voice_prompt_manifest.voice_distinctness_policy.status == DEFAULT_FALLBACK_APPLIED, audio_manifest speaker_voices and 06d expected_voices must match the fallback effective roster, not the original cloned roster
if 02c speaker_count is 3 or 4, do not apply the two-speaker lightweight similarity policy; keep the cloned roster and rely on 06d/09 QA for lineage and acoustic consistency
...
02-source-cold-open-detection/cold_open_detection_result.json exists before 02a/03, and status is CUT_CONFIRMED, NO_CONFIDENT_CUT, or an explicitly resolved manual-review state; NEEDS_VISUAL_REVIEW is not allowed past 02x in formal production
if cold_open_detection_result.json status == CUT_CONFIRMED, 02-source-cold-open-cut/cold_open_cut_validation_result.json exists with status == PASS, transcript_validation.status == PASS, first_text_validation.status == PASS, and active source files were promoted only after originals were archived as original_with_cold_open
if cold_open_detection_result.json status == NO_CONFIDENT_CUT, no source-cold-open-cut promote operation is recorded and source.mp4 remains the untrimmed original
...
selected source video height targets 1440p as the preferred quality tier; 1080p is acceptable with blocker/retry/downgrade evidence and records a QA warning; selected_height < 1080 is a FAIL for formal production
...
cover/cover_title.json highlight_texts contains 2-4 carefully chosen continuous title substrings and does not use weak generic highlights such as 外网, 播客, 视频, 中文配音
...
03d-risk-compliance-review/independent-review-result.json exists, status == PASS, and reviewed_file_hashes matches the current faithful and active translation files
...
04c text compliance summary.fail_findings == 0 and reviewed_files covers the current podcast_script.md, video_title.txt, cover/cover_title.json, final_subtitles.srt and final_subtitles.ass when those files exist
04c text compliance records reviewed_file_hashes matching the current podcast_script.md, video_title.txt, cover/cover_title.json, final_subtitles.srt and final_subtitles.ass
04c text compliance risk_registry.load_status == loaded and deterministic_rule_ids_missing_from_registry is empty
04c-bilibili-text-compliance/independent-review-result.json exists, status == PASS, and reviewed_file_hashes matches the current key publishable text
...
04d-pre-tts-frozen-contract/pre-tts-frozen-contract-result.json exists and status == PASS before 05
04d input_file_hashes match current speaker_roster, voice_prompt_manifest, active translation, script_turns, podcast_script.md, video_title.txt and cover/cover_title.json when those files exist
if any 04d input hash is stale, final QA must FAIL and the parent workflow must run plan_dependency_rebuild.py before rerunning the required downstream chain
...
chunk_plan.json chunk_count <= max_chunks_per_episode, where max_chunks_per_episode tracks the current single-video target duration, defaulting to 40 only for 30-40 minute episodes
...
subtitle_manifest.segmentation_policy.status != PASS records a QA warning, not a hard FAIL
subtitle_manifest.segmentation_policy.hard_width_fallback_count != 0 records a QA warning, not a hard FAIL
subtitle_manifest.segmentation_policy.line_violation_count != 0 records a QA warning, not a hard FAIL
subtitle_manifest.segmentation_policy.dangling_fragment_violation_count == 0
subtitle_manifest.style.max_lines != 1 records a QA warning, not a hard FAIL
subtitle_manifest.cues with line breaks or fits_single_line=false record QA warnings, not hard FAILs
...
formal source-video delivery must have render_manifest.visual_sync_mode == turn_retimed_basic_v1; disabled_v1 is a debug/exception mode and cannot pass formal 09 Final QA
render_manifest.turn_retime exists and points to 04-source-dialogue-turn-map/source_dialogue_turn_map.active.json, visual_activity.json, retime_edit_plan.json and work/source_retimed_basic.mp4
turn_retime.source_turn_map_selection == active_dialogue_turn_map; historical 02-source-turn-map, explicit debug source maps, active_mainland_safe_translation_segments, faithful_translation_segments and source_transcript_segments_fallback are invalid for formal QA
source_dialogue_turn_map.active.json exists, status == PASS, hash matches render_manifest.turn_retime.source_turn_map, and every dialogue map turn has audio_turn_id or audio_turn_ids
retime_edit_plan.json status == pass, protected_range_violation_count == 0, turn_boundary_drift_violation_count == 0, audio_turn_missing_count == 0, speaker_mismatch_count == 0, extension_policy_violation_count == 0, abs(duration_delta_vs_target_sec) <= 0.75, and cuts_per_minute <= 10
retime_edit_plan.summary.target_duration_sec matches current audio/final_podcast.wav duration plus summary.audio_start_offset_sec within 0.75s
turn_retime.retimed_video_render.duration_delta_vs_plan_sec must be within 0.75s, and turn_retime.retimed_video_render.static_video_range_check.status must be PASS
...
09a-multimodal-spot-qa/multimodal-spot-qa-result.json exists and status == PASS
09a sample_count >= 20, reviewed_sample_count == sample_count, fail_count == 0
09a required_criteria covers visual/audio semantic alignment, subtitle/audio timing, subtitle segmentation, voice identity consistency and speaker switch coherence
09a required_criteria covers video_motion_integrity, and automatic_checks.video_motion_integrity.status == PASS for formal turn-retimed outputs
09a input_file_hashes match current final_video, final_audio, render_manifest, subtitle_manifest, dialogue_timeline and 06d voice consistency result
...
QA confirms visual_sync mode: `turn_retimed_basic_v1` means basic turn-level retiming was used, without lip-sync guarantees
```
...
metadata.tags has 6-10 unique video/podcast tags
...
- 大文件上传受 VPN 明显影响时，只在“视频文件已选中、标题/简介/标签/封面/定时等字段已填好，只剩等待传输完成”的阶段临时处理 VPN。临时处理顺序：先给当前 Codex 线程设置 10/11/12 分钟左右的唤醒冗余；再只对 5 分钟主重连任务做有 TTL 的暂停：执行 `/bin/bash /Users/wangfangjia/.openclaw/workspace/scripts/vpn-auto-reconnect.sh pause-for 600`，确认 `/Users/wangfangjia/.openclaw/workspace/scripts/.vpn-auto-reconnect-paused` 存在且包含 `until_epoch=...`；然后才允许执行 Cisco VPN disconnect 并确认状态为已断开。预期行为是 10 分钟 TTL 到期后，下一次 5 分钟 LaunchAgent tick 会走 normal path，自动删除过期 pause marker，并在仍断开时重连 VPN；10/11/12 分钟唤醒只负责回到 B 站上传页轮询 `上传完成`，不负责长期维持 VPN 断开。严禁修改、暂停、卸载或重写 40 分钟兜底任务；该任务的历史文件名仍为 `/Users/wangfangjia/Library/LaunchAgents/com.user.cisco-vpn-auto-hourly-backup.plist` 与 `/Users/wangfangjia/.openclaw/workspace/scripts/vpn-auto-reconnect-hourly-backup.sh`。不要把 `/Users/wangfangjia/Library/LaunchAgents/com.user.cisco-vpn-auto.plist` 改为 `StartInterval=600`、`RunAtLoad=false` 当作暂停；那只是延迟重连，不是硬禁用。若有 TTL 的主任务暂停或验证失败，必须停止并报告 BLOCKED，不得断开 VPN。旧的裸 `/bin/bash .../vpn-auto-reconnect.sh pause` 是无限期人工暂停，除非 `pause-for` 不可用且同一 turn 能保证 `resume`，否则不得用于 B 站大文件上传。40 分钟兜底任务在超长上传中可能按设计把 VPN 接回，不得为上传禁用它。
- 上传完成或全系列提交后，必须确认 5 分钟主任务 plist 仍为默认 `StartInterval=300`、`RunAtLoad=true`；如果本流程曾卸载主 LaunchAgent，则重新加载主 LaunchAgent；如果 `/Users/wangfangjia/.openclaw/workspace/scripts/.vpn-auto-reconnect-paused` 仍存在，再执行 `/bin/bash /Users/wangfangjia/.openclaw/workspace/scripts/vpn-auto-reconnect.sh resume`，让 Codex 的 5 分钟 VPN 保障回到常态。若页面已经明确显示 `上传完成`，不要再做无意义上传进度轮询，直接做一次字段/封面/定时/提交证据验证。
...
1. 从 `SUBMITTED` 的提交时间算起，等待 10 分钟后检查审核状态；若恢复任务时提交时间已经超过 10 分钟，则立即执行第一次检查。
2. 若第一次检查显示审核通过/已发布，写 `APPROVED` 报告并结束。
3. 若第一次检查仍是 `稿件审核 进行中` / `审核中` / `待审核`，再等 10 分钟做第二次检查。
...
if audit_monitor_report.status == BLOCKED or UNKNOWN, parent workflow must first run the audit recovery steps below; only after repeated classification attempts fail may it report the exact blocker and keep the Bilibili tab open
```
...
- 修复后重新提交 B 站必须重置 11c：从新的 `SUBMITTED` 时间重新等待 10 分钟检查，最多两次。新退回原因继续追加到 lesson 文件，直到 `APPROVED` 或 `REVIEW_PENDING_AFTER_MAX_CHECKS`。

BLOCKED/UNKNOWN 恢复循环：

- 如果缺 `bvid` 或进度 URL，先从 B 站成功页、稿件管理页或上传报告中的标题和提交时间找回同标题稿件，把 `bvid` 回写到 `bilibili_upload_draft_report.json` 和 11c report，再重试监控。
- 如果页面状态不明确，刷新进度页、打开稿件管理列表交叉验证标题/封面/提交时间，并记录 visible text；不要因为第一次 DOM 选择器没命中就结束。
- 如果遇到登录、验证码、网络、Chrome 控制或标签页丢失问题，先按 Chrome/Bilibili upload skill 的边界恢复真实 Chrome 会话；能打开稿件管理页后重新分类。只有需要用户亲自完成登录/验证码或平台长期不可达时，才报告 BLOCKED。
- 每次恢复尝试必须追加到 `bilibili_audit_monitor_report.json.checks[]` 或 markdown 报告；同一状态至少完成两次不同证据路径的分类尝试后，才允许把 `UNKNOWN` 交给用户。

### 11d Post-Run Process Review And Skill Optimization

11c 达到允许终态后，必须运行一次流程复盘节点，然后才允许进入 `12 Series Final QA`。本节点的目标不是评价视频内容，而是复盘本次生产流程：哪里耗时最长、哪里发生了返工、这些返工是否来自流程设计缺陷，以及是否存在证据充分、低风险、可以立刻落地的 skill/script 优化。

触发条件：

```text
11c-bilibili-audit-monitor/bilibili_audit_monitor_report.json exists
11c status is APPROVED or REVIEW_PENDING_AFTER_MAX_CHECKS
or, if 11c returned and episode was repaired/resubmitted, the latest 11c status is APPROVED or REVIEW_PENDING_AFTER_MAX_CHECKS
```

输出：

```text
11d-process-review/process-review-result.json
11d-process-review/process-review-report.md
```

复盘必须读取并引用证据，不得只凭主观印象：

- `logs/progress.md`、各节点 report/manifest 的 `created_at`、`elapsed_sec`、`duration_sec`、`generation_sec`、`rtf`、`checks[]` 和失败/返工记录。
- `05-vibevoice-chunks/resident_batch_report.json`、`audio_report.md`、chunk plan、repair batch/report。
- `06b/06c/06d/07/08/09a/09/10/11/11c/12` 的结果文件、warnings、failures、repair attempts 和 stale hash 信息。
- B 站上传报告里的上传耗时、VPN 处理、字段返工、最终提交和审核监控耗时。

至少分析：

```text
longest_phase
top_time_consuming_nodes
all_rework_loops
owner_node_for_each_rework
whether_the_rework_was_avoidable_by_an_upstream_gate
whether_the_current_skill_or_script_already_prevents_this_issue
...
`PASS_OPTIMIZED` 只允许在以下条件同时满足时使用：

- 有明确证据证明某个节点耗时最长、反复返工，或同类失败在历史 run 中重复出现。
- 能定位到具体 owner node、skill 段落、脚本、测试或门禁，而不是泛泛说“模型不稳定”。
- 优化不会降低质量门禁，不会隐藏真实失败，不会跳过 03c/03d/04c/06b/06d/09a/09/11c 等硬门禁。
- 修改范围可控，且执行 agent 能在本轮完成对应 skill/script/test 修改。
- 修改后至少运行相关测试；若修改主 skill 或关键脚本，还必须按“开发验证纪律”启动独立执行/评估子 agent 或记录为什么本次只允许静态规则更新。

如果证据不足、改动收益不确定、需要用户产品判断、需要更大重构、或可能降低质量门禁，必须选择 `PASS_OPTIMIZATION_DECLINED`，并写明拒绝优化的证据和下一步观察点。如果没有发现可行动的流程问题，选择 `PASS_NO_ACTIONABLE_OPTIMIZATION`。不得为了让流程显得“自动优化”而做无证据、无测试或只改文案的假优化。

`process-review-result.json` 至少包含：

```json
{
  "schema_version": "worldview-china-process-review.v1",
  "status": "PASS_OPTIMIZED | PASS_NO_ACTIONABLE_OPTIMIZATION | PASS_OPTIMIZATION_DECLINED",
  "optimization_decision": "optimized | no_actionable_optimization | declined_due_to_insufficient_evidence",
  "longest_phase": {
    "node": "05-vibevoice-chunks",
    "duration_sec": 0,
    "evidence": "path or short evidence note"
  },
  "evidence_sources": ["paths inspected"],
  "top_time_consuming_nodes": [],
  "rework_loops": [],
  "candidate_optimizations": [],
  "applied_changes": [],
...
```text
11d-process-review/process-review-result.json exists
status is PASS_OPTIMIZED, PASS_NO_ACTIONABLE_OPTIMIZATION, or PASS_OPTIMIZATION_DECLINED
optimization_decision matches status
longest_phase is present
evidence_sources is a non-empty list of inspected artifacts
if status == PASS_OPTIMIZED, applied_changes and tests_run are non-empty
```

本节点可以产出新的 skill/script 修改，但这些修改属于“下一轮更好”，不能倒改本次已经通过审核的视频事实。若 11d 修改了上游生产节点规则，不要求回头重做本次已发布视频；但必须在 `process-review-report.md` 说明该优化从下一次 run 生效。

### 12 Series Final QA

如果本轮启用了 `04b-series-episodes`，所有 episode 完成 09/10/11a，11b 统一最终提交完成，11c 审核监控达到允许终态，且 11d 流程复盘达到允许终态后，必须在父 run 上运行系列最终门禁：
...
默认正式生产要求每个 episode 的 `bilibili_upload_draft_report.json.status == SUBMITTED`，并且 `11c-bilibili-audit-monitor/bilibili_audit_monitor_report.json.status` 是 `APPROVED` 或 `REVIEW_PENDING_AFTER_MAX_CHECKS`，且 `11d-process-review/process-review-result.json.status` 是 `PASS_OPTIMIZED`、`PASS_NO_ACTIONABLE_OPTIMIZATION` 或 `PASS_OPTIMIZATION_DECLINED`。`READY_TO_SUBMIT` 只允许作为 11a 和跨 episode 重叠生产的中间态，不能让 `12 Series Final QA` 通过。`RETURNED_NEEDS_REPAIR`、`BLOCKED` 或 `UNKNOWN` 不能通过系列最终 QA；必须把失败 episode 当作修复队列，先按 11/11c playbook 修复、重投并重新跑 11c 与 11d。只有调试或用户明确允许“上传阻塞也先验收机械链路”时，才可传 `--allow-blocked-upload`；该模式不得声称正式发布完成。
...
4. 每个 episode 已完成 11d 流程复盘：process review status == PASS_OPTIMIZED、PASS_NO_ACTIONABLE_OPTIMIZATION 或 PASS_OPTIMIZATION_DECLINED，且有最长耗时阶段和证据来源
```
...
each episode 11d-process-review/process-review-result.json exists
each episode process review status is PASS_OPTIMIZED, PASS_NO_ACTIONABLE_OPTIMIZATION, or PASS_OPTIMIZATION_DECLINED
each episode process review records optimization_decision, longest_phase, and non-empty evidence_sources
```
...
- 流程复盘报告 `11d-process-review/process-review-result.json` 路径、最长耗时阶段、优化决策和是否修改了 skill/script。
