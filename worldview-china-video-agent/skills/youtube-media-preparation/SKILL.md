...
2026-06-22 起，用户已对本机 Worldview China 播客生产流程给出长期授权：如果 YouTube 元信息、字幕、格式列表或下载返回 bot/sign-in/IP 风控，直接用 `--cookies-from-browser chrome` 重试，不再询问。仍然禁止读取、导出、打印或保存 cookie 原文；`download.log` 和 `media_manifest.json` 只能记录脱敏状态，例如 `cookies_from_browser=chrome` / `provided`。
